...
export const functions = {
  ...
  ITEM_TYPES(ctx, CFG, arr) {
    let all = arr
      .map(item =>
        item.startsWith("tag:")
          ? ctx.types
              .filter(t => t.tags && t.tags.includes(item.slice(4)))
              .map(t => t.name)
          : item
      )
      .flat();
  ...
  GET_HOUR(ctx, CFG) {
    return ((ctx.turn / CFG.TURNS_PER_HOUR) >> 0) % 24;
  },
  GET_DAY(ctx, CFG) {
    return (1 + ctx.turn / CFG.TURNS_PER_HOUR / 24) >> 0;
  },
  ...
  TRADER_ITEMS(ctx, CFG) {
    return ctx.types
      .filter(type =>
        ctx.trader.sells.some(
          selling =>
            selling === type.name ||
            (selling.startsWith("tag:") &&
              type.tags &&
              type.tags.includes(selling.slice(4)))
        )
      )
      .map(type => type.name);
  },
  ...
  COMBAT_IS_FINISHED(ctx, CFG) {
    return (
      ctx.combat_won ||
      ctx.combat_lost ||
      ctx.stats.energy < 1 ||
      ctx.opponent.stats.energy < 1
    );
  },
  COMBAT_ATTACK(ctx, CFG) {
    ...
    this.MSG(
      ctx,
      CFG,
      `You hit ${ctx.opponent.name} for ${player_dmg} damage. `,
      "combat"
    );
  },
